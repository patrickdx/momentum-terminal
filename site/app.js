...
function renderRadar() {
  ...
  $('#radar-chart').innerHTML=`<svg viewBox="0 0 1100 400" role="group" aria-label="Industry momentum versus trend breadth. Circle size represents member count. Colors match the legend."><text x="20" y="20" fill="#929da5" font-size="12">MOMENTUM SCORE</text>${[0,25,50,75,100].map(n=>`<line x1="60" x2="750" y1="${350-n*3}" y2="${350-n*3}" stroke="#263037"/><text x="42" y="${354-n*3}" text-anchor="end" fill="#929da5" font-size="12">${n}</text><line x1="${60+n*6.9}" x2="${60+n*6.9}" y1="50" y2="350" stroke="#202930"/><text x="${60+n*6.9}" y="375" text-anchor="middle" fill="#929da5" font-size="12">${n}%</text>`).join('')}<text x="750" y="396" text-anchor="end" fill="#929da5" font-size="12">TREND BREADTH →</text><text x="790" y="40" fill="#929da5" font-size="12">INDUSTRIES / CLICK TO EXPLORE</text>${themes.map((t,i)=>`<g data-radar-theme="${esc(t.name)}" tabindex="0" role="button" aria-label="Explore ${esc(t.name)}" style="cursor:pointer"><circle cx="${60+t.breadth*6.9}" cy="${350-t.score*3}" r="${Math.min(25,7+Math.sqrt(t.count))}" fill="${colors[i%colors.length]}" fill-opacity=".22" stroke="${colors[i%colors.length]}" stroke-width="1.5"><title>${esc(t.name)}: score ${t.score}, breadth ${num(t.breadth,0)}%, ${t.count} stocks</title></circle><circle cx="798" cy="${66+i*29}" r="4" fill="${colors[i%colors.length]}"/><text x="815" y="${70+i*29}" fill="${colors[i%colors.length]}" font-size="13">${esc(t.name)}</text></g>`).join('')}</svg>`;
...
$('#radar-chart').addEventListener('click',e=>{const item=e.target.closest('[data-radar-theme]');if(item){state.theme=item.dataset.radarTheme;state.view='screener';clearStockFilters();renderMarket();$('#screen-view').scrollIntoView({block:'start'});}});
$('#radar-chart').addEventListener('keydown',e=>{if(['Enter',' '].includes(e.key)&&e.target.closest('[data-radar-theme]')){e.preventDefault();e.target.dispatchEvent(new MouseEvent('click',{bubbles:true}));}});
